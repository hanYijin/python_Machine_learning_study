...
def get_current_movies():
	...
	#select Xpath// *[ @ id = "content"] / div[1] / div[1] / div[3]
	# content > div.article > div:nth-child(1) > div.lst_wrap
	# 3. 필요한 정보만 추출
	movies = []
	for idx, li in enumerate(lis):
		...
		temp['img']=li.select('dl > a > img')[0].text
		...
		if idx > 10:
			break	
	return movies 		
# def get_rank_keyword():
# 	# 1. URL로부터 html을 파싱해오기
# 	req = requests.get(URL)
# 	html = req.text
# 	soup = BeautifulSoup(html, 'html.parser')
# 	# 2. 영화 부분 추출하기
# 	lis = soup.select('#obj_section > div.rank_keyword > div:nth-of-type(1) > div.lst_wrap > ul > li')
# 	# 3. 필요한 정보만 추출
# 	movies = []
# 	for idx, li in enumerate(lis):
# 		temp = {}
